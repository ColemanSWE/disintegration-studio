...
import { applyEffects, isStyleEffect, getProcessingEffects } from './effect-registry';
...
function styleEffectToRenderMode(type: string): RenderMode | 'halftone' {
  switch (type) {
    case 'pixelate': return 'standard';
    case 'emoji': return 'emoji';
    case 'ascii': return 'ascii-color';
...
export async function initializeEmojiPalette(paletteKey: PaletteKey): Promise<void> {
...
export function renderWithEffects(
  ...
  const { ctx, buffer, bufferCtx, source, sourceWidth, sourceHeight, canvasWidth, canvasHeight, tick } = context;
  ...
  if (renderMode === 'halftone') {
    ...
    renderHalftone(renderCtx, imageData.data, cols, rows, canvasWidth, canvasHeight, fontSize, dotScale);
  } else {
  ...
  if (postStyleEffects.length > 0) {
    const renderImageData = renderCtx.getImageData(0, 0, canvasWidth, canvasHeight);
